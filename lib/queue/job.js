...
Job.prototype.delay = function(ms){
  ...
  this._state = 'delayed';
  // serialize Infinity
  // the exponent is too large to represent in a double
  // and when deserialized this is represented as Infinity
  if(this._delay === Infinity) this._delay = '1.0e+1024'
  return this;
};
...
Job.prototype.state = function(state){
  ...
  if ('failed' === state) this.emit('failed',new Error('User set the job to failed.'));
  return this;
};
...
Job.prototype.complete = function(){
  // Remove the job from the sorted set by priority
  this.client.zrem('q:job:priorities', this.id);
  return this.set('progress', 100).state('complete');
};

/**
 * Set state to "failed, and remove the job from the sorted set by priority
 */

Job.prototype.completeFailure = function(){
  // Remove the job from the sorted set by priority
  this.client.zrem('q:job:priorities', this.id);
...
Job.prototype.update = function(fn){
  ...
  // Add the job id into a sorted set by priority
  client.zadd('q:job:priorities', this._priority, this.id,function(err){
    if(err) return fn(err);
  });
