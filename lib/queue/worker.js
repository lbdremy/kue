...
Worker.prototype.process = function(job, fn){
  ...
  fn(job, function(err){
    ...
    // Remove the job from the sorted set by priorities 
    self.client.zrem('q:job:priorities', job.id, function(err){
      self.start(fn); 
    });
  });
  return this;
  ...
  client.blpop('q:' + self.type + ':jobs', 0, function(err) {
    self.zpop('q:jobs:' + self.type + ':inactive', function(err, id){
      ...
      Job.get(id, function(err,job){
        if(err) return fn(err);
        // Consider priorities of all `type` of jobs
        client.zrangebyscore('q:job:priorities','-inf', job._priority - 1,function(err,members){
          if(err) return fn(err);
          if(members && members.length > 0){
            client.zadd('q:jobs:' + self.type + ':inactive', job._priority, job.id,function(err){
              if(err) console.error(err);
              // increase available jobs, since i'm gonna abort the process of this job
