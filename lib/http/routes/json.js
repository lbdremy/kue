...
exports.create = function(req,res){
  ...
  if(!type) return res.send(400,{err : 'creation of a job required a type.'});
  ...
  job.save(function(err){
    if(err) return res.send(500, { err : err.name + ': ' + err.message });
    res.send(200, { messsage : 'job ' + job.id + 'created'});
  });
}
